...
if (valorFerias.valor <= 0) {
    if (folha.folhaPagamento) {
        ...
        if (vvar >= 0) {
            ...
            double valorTotal
            if (tipo == 1) {
                valorTotal = dependentes * EncargosSociais.SalarioFamilia.Especial.buscaMaior(2)
            } else {
            ...
            if (matricula.possuiMultiploVinculo) {
                double valorCalculadoMultiplosVinculos = Eventos.valorCalculadoMultiplosVinculos(ClassificacaoEvento.SALFAMESP, TipoValor.CALCULADO, calculo.tipoProcessamento, calculo.subTipoProcessamento)
                if (SubTipoProcessamento.COMPLEMENTAR.equals(calculo.subTipoProcessamento)) {
                    valorCalculadoMultiplosVinculos += Eventos.valorCalculadoMultiplosVinculos(ClassificacaoEvento.SALFAMESP, TipoValor.CALCULADO, calculo.tipoProcessamento, SubTipoProcessamento.INTEGRAL)
                }
                if (TipoProcessamento.RESCISAO.equals(calculo.tipoProcessamento)) {
                    valorCalculadoMultiplosVinculos += Eventos.valorCalculadoMultiplosVinculos(ClassificacaoEvento.SALFAMESP, TipoValor.CALCULADO, TipoProcessamento.FERIAS, SubTipoProcessamento.INTEGRAL)
                    valorCalculadoMultiplosVinculos += Eventos.valorCalculadoMultiplosVinculos(ClassificacaoEvento.SALFAMESP, TipoValor.CALCULADO, TipoProcessamento.MENSAL, SubTipoProcessamento.INTEGRAL)
                    valorCalculadoMultiplosVinculos += Eventos.valorCalculadoMultiplosVinculos(ClassificacaoEvento.SALFAMESP, TipoValor.CALCULADO, TipoProcessamento.MENSAL, SubTipoProcessamento.COMPLEMENTAR)
                }
                if (TipoProcessamento.MENSAL.equals(calculo.tipoProcessamento)) {
                    valorCalculadoMultiplosVinculos += Eventos.valorCalculadoMultiplosVinculos(ClassificacaoEvento.SALFAMESP, TipoValor.CALCULADO, TipoProcessamento.FERIAS, SubTipoProcessamento.INTEGRAL)
                    valorCalculadoMultiplosVinculos += Eventos.valorCalculadoMultiplosVinculos(ClassificacaoEvento.SALFAMESP, TipoValor.CALCULADO, TipoProcessamento.RESCISAO, SubTipoProcessamento.INTEGRAL)
                    valorCalculadoMultiplosVinculos += Eventos.valorCalculadoMultiplosVinculos(ClassificacaoEvento.SALFAMESP, TipoValor.CALCULADO, TipoProcessamento.RESCISAO, SubTipoProcessamento.COMPLEMENTAR)
                }
