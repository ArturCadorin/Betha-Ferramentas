...
if (vvar >= 0) {
    ...
    if (possuiMultiplosVinculos) {
        if (!deducaoSimplificada13IntegralAntecipado &&
                (Eventos.valorCalculadoMultiplosVinculos(ClassificacaoEvento.DEDUCSIMPLIRRF13, TipoValor.CALCULADO, calculo.tipoProcessamento, calculo.subTipoProcessamento) > 0
                || Eventos.valorCalculadoMultiplosVinculos(ClassificacaoEvento.DEDUCCOMPLIRRF13, TipoValor.CALCULADO, calculo.tipoProcessamento, calculo.subTipoProcessamento) > 0)) {
            suspender 'Já há dedução de IRRF 13º salário aplicada na competência para este servidor neste processamento'
        }
        if (SubTipoProcessamento.COMPLEMENTAR.equals(calculo.subTipoProcessamento)) {
            if (Eventos.valorCalculadoMultiplosVinculos(ClassificacaoEvento.DEDUCSIMPLIRRF13, TipoValor.CALCULADO, calculo.tipoProcessamento, SubTipoProcessamento.INTEGRAL) > 0
                || Eventos.valorCalculadoMultiplosVinculos(ClassificacaoEvento.DEDUCCOMPLIRRF13, TipoValor.CALCULADO, calculo.tipoProcessamento, SubTipoProcessamento.INTEGRAL) > 0) {
                suspender 'Já há dedução de IRRF 13º salário aplicada na competência para este servidor em subtipo de processamento integral anterior'
            }
        }
    }
    ...
    if (EncargosSociais.IRRF.deducaoSimplificadaIrrf <= baseDeducaoIrrf13) {
        suspender 'O valor da dedução simplificada é menor ou igual ao valor das deduções legais do 13º salário. Por este motivo, deve ser aplicada a dedução legal para o 13º salário'
    }
